...
# libFBCARun.runFBCA(S, SMAT, NEIGHBOURHOOD, steps=GENS, show = True, showFinal = True, filename = f"test", colours = COLOURS, fixedRNG = True)

# SMAT = np.array([[-6.309314623420491, 4.470947446514341], [13.929052553485665, -12.090685376579515]]) #9 2

# libFBCARun.runFBCA(S, SMAT, NEIGHBOURHOOD, steps=GENS, show = True, showFinal = True, filename = f"test2", colours = COLOURS, fixedRNG = True)

# SMAT = np.array([[-5.805197807268216, 6.181708963575444], [12.61829103642456, -12.994802192731788]]) #11 1

# libFBCARun.runFBCA(S, SMAT, NEIGHBOURHOOD, steps=GENS, show = True, showFinal = True, filename = f"test3", colours = COLOURS, fixedRNG = True)

# SMAT = np.array([[-4.000265957525192, 4.1007922878948815], [13.499207712105127, -13.599734042474816]]) #11 2

# libFBCARun.runFBCA(S, SMAT, NEIGHBOURHOOD, steps=GENS, show = True, showFinal = True, filename = f"test4", colours = COLOURS, fixedRNG = True)
